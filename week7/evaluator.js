...
export class Evaluator {
    ...
    evaluateModule(node) {
        ...
        let newEC = new ExecutionContext(this.realm, new EnvironmentRecord(globalEC.lexicalEnvironment),
            new EnvironmentRecord(globalEC.lexicalEnvironment));
        this.ecs.push(newEC);
        let result = this.evaluateModule(node);
        this.ecs.pop();
        return result;
    }
    ...
    Arguments(node) {
        if (node.children.length === 2)
            return [];
        else return this.evaluate(node.v = children[1]);
    }
    ArgumentList(node) {
        if (node.children.length === 1) {
            let result = this.evaluate(code.children[0]);
